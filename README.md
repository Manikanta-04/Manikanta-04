<div align="center">

<img src="https://capsule-render.vercel.app/api?type=waving&color=gradient&customColorList=6,11,20&height=180&section=header&text=Naripeddi%20Manikanta&fontSize=42&fontColor=fff&animation=twinkling&fontAlignY=32&desc=Full%20Stack%20Developer%20%7C%20ML%20Engineer%20%7C%20Creative%20Builder&descAlignY=55&descSize=18"/>

</div>

<div align="center">

[![Typing SVG](https://readme-typing-svg.demolab.com?font=Fira+Code&weight=600&size=22&duration=3000&pause=1000&color=6366F1&center=true&vCenter=true&multiline=true&width=700&height=100&lines=Building+full-stack+products+from+0+to+production+%F0%9F%9A%80;ML+%2B+React+%2B+Node.js+%2B+Python+%F0%9F%A7%A0;Turning+ideas+into+live%2C+deployed+apps+%F0%9F%8C%8D)](https://git.io/typing-svg)

</div>

---

<div align="center">

[![LinkedIn](https://img.shields.io/badge/LinkedIn-Manikanta%20Naripeddi-0077B5?style=for-the-badge&logo=linkedin&logoColor=white)](https://linkedin.com/in/manikanta-naripeddi-4326232a5/)
[![GitHub](https://img.shields.io/badge/GitHub-Manikanta--04-181717?style=for-the-badge&logo=github&logoColor=white)](https://github.com/Manikanta-04)
[![YouTube](https://img.shields.io/badge/YouTube-@ManikantaNaripeddi-FF0000?style=for-the-badge&logo=youtube&logoColor=white)](https://youtube.com/@ManikantaNaripeddi)
[![Instagram](https://img.shields.io/badge/Instagram-manikanta__naripeddi-E4405F?style=for-the-badge&logo=instagram&logoColor=white)](https://instagram.com/manikanta_naripeddi)
[![Email](https://img.shields.io/badge/Email-manikantachowdary296@gmail.com-D14836?style=for-the-badge&logo=gmail&logoColor=white)](mailto:manikantachowdary296@gmail.com)

</div>

---

## 👨‍💻 About Me

```python
class Manikanta:
    name       = "Naripeddi Manikanta"
    college    = "VRSEC IT — Batch 2027"
    role       = "Full Stack Developer & ML Engineer"
    location   = "Hyderabad, India 🇮🇳"

    languages  = ["Python", "JavaScript", "Java", "C"]
    frontend   = ["React", "Vite", "Tailwind CSS", "Three.js", "GSAP"]
    backend    = ["Node.js", "Express.js", "FastAPI", "Flask", "Spring Boot"]
    databases  = ["MongoDB Atlas", "MySQL", "Supabase"]
    ml_stack   = ["scikit-learn", "XGBoost", "TensorFlow", "Pandas", "NumPy"]
    deployment = ["Vercel", "Render", "AWS", "GitHub Pages", "Streamlit Cloud"]

    currently  = "Building AI-powered disaster management + healthcare systems"
    seeking    = "SDE / Full Stack / ML Engineer roles — 2027 batch"
    fun_fact   = "I build every project end-to-end: idea → code → deploy → README 😄"
```

---

## 🚀 Featured Projects

<table>
<tr>
<td width="50%">

### 🌍 DRISHTI — Disaster Command Platform
AI-powered real-time disaster management with ML predictions, live alerts, Socket.io, and interactive maps.

**Stack:** React · Node.js · FastAPI · MongoDB · scikit-learn

[![Live](https://img.shields.io/badge/Live-Demo-brightgreen?style=flat-square)](https://drishti-disaster-command.vercel.app)
[![Repo](https://img.shields.io/badge/GitHub-Repo-181717?style=flat-square&logo=github)](https://github.com/Manikanta-04/drishti-disaster-command)

</td>
<td width="50%">

### 🚑 Emergency Hospital Availability System
AI hospital routing — routes ambulances to the *right* hospital using Distance + Specialist + Bed scoring.

**Stack:** React · Node.js · MongoDB · n8n · Socket.io · Leaflet.js

[![Live](https://img.shields.io/badge/Live-Demo-brightgreen?style=flat-square)](https://emergency-hospital-system.vercel.app)
[![Repo](https://img.shields.io/badge/GitHub-Repo-181717?style=flat-square&logo=github)](https://github.com/Manikanta-04/emergency-hospital-system)

</td>
</tr>
<tr>
<td width="50%">

### 🏉 PKL Match Winner Predictor
ML web app predicting Pro Kabaddi League match outcomes — 59.85% accuracy with Gradient Boosting.

**Stack:** Python · Streamlit · scikit-learn · Pandas

[![Live](https://img.shields.io/badge/Live-Demo-brightgreen?style=flat-square)](https://pkl-predictor.onrender.com)
[![Repo](https://img.shields.io/badge/GitHub-Repo-181717?style=flat-square&logo=github)](https://github.com/Manikanta-04/PKL_PREDICTION)

</td>
<td width="50%">

### 🎓 Student Salary Prediction
Predicts expected salary from CGPA, LeetCode, GitHub, projects & skills — 81% R² (Gradient Boosting).

**Stack:** Python · Streamlit · scikit-learn · XGBoost

[![Live](https://img.shields.io/badge/Live-Demo-brightgreen?style=flat-square)](https://studentsalaryprediction-manikanta.streamlit.app)
[![Repo](https://img.shields.io/badge/GitHub-Repo-181717?style=flat-square&logo=github)](https://github.com/Manikanta-04/Student_Salary_prediction)

</td>
</tr>
<tr>
<td width="50%">

### 🌌 Galaxy Relay — Omnitrix Chat
Real-time global group chat with presence, typing indicators, reactions, and Omnitrix-themed UI.

**Stack:** HTML · CSS · JavaScript · Supabase Realtime

[![Live](https://img.shields.io/badge/Live-Demo-brightgreen?style=flat-square)](https://galaxy-relay-themed-online-chat.vercel.app)
[![Repo](https://img.shields.io/badge/GitHub-Repo-181717?style=flat-square&logo=github)](https://github.com/Manikanta-04/Galaxy_relay-themed_online_chat)

</td>
<td width="50%">

### 🌲 ArborVeil — 3D Forest Landing Page
Photorealistic 3D forest with real-world wind physics, 2000-particle pollen, god-ray lighting — single HTML file.

**Stack:** Three.js · GSAP · Vanilla JS · HTML/CSS

[![Live](https://img.shields.io/badge/Live-Demo-brightgreen?style=flat-square)](https://manikanta-04.github.io/Forest_management/)
[![Repo](https://img.shields.io/badge/GitHub-Repo-181717?style=flat-square&logo=github)](https://github.com/Manikanta-04/Forest_management)

</td>
</tr>
</table>

> 📌 View all → [github.com/Manikanta-04?tab=repositories](https://github.com/Manikanta-04?tab=repositories)

---

## 🛠️ Tech Stack

### 🌐 Frontend
![React](https://img.shields.io/badge/React-20232a?style=for-the-badge&logo=react&logoColor=61DAFB)
![Vite](https://img.shields.io/badge/Vite-646CFF?style=for-the-badge&logo=vite&logoColor=white)
![HTML5](https://img.shields.io/badge/HTML5-E34F26?style=for-the-badge&logo=html5&logoColor=white)
![CSS3](https://img.shields.io/badge/CSS3-1572B6?style=for-the-badge&logo=css3&logoColor=white)
![Three.js](https://img.shields.io/badge/Three.js-000000?style=for-the-badge&logo=three.js&logoColor=white)
![GSAP](https://img.shields.io/badge/GSAP-88CE02?style=for-the-badge)

### ⚙️ Backend
![Node.js](https://img.shields.io/badge/Node.js-339933?style=for-the-badge&logo=node.js&logoColor=white)
![Express.js](https://img.shields.io/badge/Express.js-404d59?style=for-the-badge&logo=express&logoColor=white)
![FastAPI](https://img.shields.io/badge/FastAPI-005571?style=for-the-badge&logo=fastapi)
![Flask](https://img.shields.io/badge/Flask-000000?style=for-the-badge&logo=flask&logoColor=white)
![Spring Boot](https://img.shields.io/badge/Spring_Boot-6DB33F?style=for-the-badge&logo=spring&logoColor=white)
![Socket.io](https://img.shields.io/badge/Socket.io-010101?style=for-the-badge&logo=socket.io)

### 🗄️ Databases & Cloud
![MongoDB](https://img.shields.io/badge/MongoDB-47A248?style=for-the-badge&logo=mongodb&logoColor=white)
![MySQL](https://img.shields.io/badge/MySQL-4479A1?style=for-the-badge&logo=mysql&logoColor=white)
![Supabase](https://img.shields.io/badge/Supabase-3ECF8E?style=for-the-badge&logo=supabase&logoColor=white)
![AWS](https://img.shields.io/badge/AWS-FF9900?style=for-the-badge&logo=amazon-aws&logoColor=white)

### 🤖 ML / Data Science
![Python](https://img.shields.io/badge/Python-3776AB?style=for-the-badge&logo=python&logoColor=white)
![scikit-learn](https://img.shields.io/badge/scikit--learn-F7931E?style=for-the-badge&logo=scikit-learn&logoColor=white)
![XGBoost](https://img.shields.io/badge/XGBoost-FF0000?style=for-the-badge)
![TensorFlow](https://img.shields.io/badge/TensorFlow-FF6F00?style=for-the-badge&logo=tensorflow&logoColor=white)
![Pandas](https://img.shields.io/badge/Pandas-150458?style=for-the-badge&logo=pandas&logoColor=white)
![NumPy](https://img.shields.io/badge/NumPy-013243?style=for-the-badge&logo=numpy&logoColor=white)
![Streamlit](https://img.shields.io/badge/Streamlit-FE4B4B?style=for-the-badge&logo=streamlit&logoColor=white)

### 🚀 DevOps & Tools
![Git](https://img.shields.io/badge/Git-F05033?style=for-the-badge&logo=git&logoColor=white)
![GitHub Actions](https://img.shields.io/badge/GitHub_Actions-2671E5?style=for-the-badge&logo=githubactions&logoColor=white)
![Vercel](https://img.shields.io/badge/Vercel-000000?style=for-the-badge&logo=vercel&logoColor=white)
![Render](https://img.shields.io/badge/Render-46E3B7?style=for-the-badge&logo=render&logoColor=white)

---

## 📊 GitHub Stats

<div align="center">

<img height="180em" src="https://github-readme-stats.vercel.app/api?username=Manikanta-04&show_icons=true&theme=tokyonight&hide_border=true&include_all_commits=true&count_private=true&rank_icon=github" />
<img height="180em" src="https://github-readme-stats.vercel.app/api/top-langs/?username=Manikanta-04&theme=tokyonight&hide_border=true&layout=compact&langs_count=8" />

</div>

<div align="center">

<img width="70%" src="https://streak-stats.demolab.com?user=Manikanta-04&theme=tokyonight&hide_border=true&date_format=j%20M%5B%20Y%5D" />

</div>

---

## 🏆 GitHub Trophies

<div align="center">

<img src="https://github-profile-trophy.vercel.app/?username=Manikanta-04&theme=discord&no-frame=true&no-bg=true&margin-w=4&column=4&row=2" />

</div>

---

## 📈 Contribution Graph

<div align="center">

<img src="https://github-readme-activity-graph.vercel.app/graph?username=Manikanta-04&theme=tokyo-night&hide_border=true&area=true" />

</div>

---

## 🎯 What I'm Currently Working On

```
🔭  Building   — DRISHTI v2 with satellite imagery + deep learning models
🌱  Learning   — AWS Solutions Architect + System Design
👯  Open to    — Hackathons, open source collabs, internship opportunities
💬  Ask me     — Full stack architecture, ML deployment, React, Node.js
📫  Reach me   — manikantachowdary296@gmail.com
⚡  Fun fact   — I deploy every project I build. No "WIP" repos here.
```

---

## 🏅 Certifications & Achievements

- 🏆 Built & deployed **10+ full-stack and ML projects** — all live in production
- 🌍 **DRISHTI** — Disaster management platform built for India's emergency response
- 🚑 **Emergency Hospital System** — AI routing covering 1,234+ real hospitals (AP + TN)
- 🤖 ML projects achieving **81% R² accuracy** (Student Salary Prediction)
- ⚡ **CODEBID** — Real-time competitive coding platform built with MERN + Socket.io
- 🎓 VRSEC IT — Batch 2027 | Roll: **238W1A12A7**

---

## ✍️ Random Dev Quote

<div align="center">

![](https://quotes-github-readme.vercel.app/api?type=horizontal&theme=tokyonight)

</div>

---

## 🔝 Top Contributed Repos

<div align="center">

| Repository | Stars | Forks |
|---|---|---|
| [🌍 drishti-disaster-command](https://github.com/Manikanta-04/drishti-disaster-command) | ![Stars](https://img.shields.io/github/stars/Manikanta-04/drishti-disaster-command?style=flat-square&color=6366f1) | ![Forks](https://img.shields.io/github/forks/Manikanta-04/drishti-disaster-command?style=flat-square&color=6366f1) |
| [🚑 emergency-hospital-system](https://github.com/Manikanta-04/emergency-hospital-system) | ![Stars](https://img.shields.io/github/stars/Manikanta-04/emergency-hospital-system?style=flat-square&color=6366f1) | ![Forks](https://img.shields.io/github/forks/Manikanta-04/emergency-hospital-system?style=flat-square&color=6366f1) |
| [🏉 PKL_PREDICTION](https://github.com/Manikanta-04/PKL_PREDICTION) | ![Stars](https://img.shields.io/github/stars/Manikanta-04/PKL_PREDICTION?style=flat-square&color=6366f1) | ![Forks](https://img.shields.io/github/forks/Manikanta-04/PKL_PREDICTION?style=flat-square&color=6366f1) |
| [🎓 Student_Salary_prediction](https://github.com/Manikanta-04/Student_Salary_prediction) | ![Stars](https://img.shields.io/github/stars/Manikanta-04/Student_Salary_prediction?style=flat-square&color=6366f1) | ![Forks](https://img.shields.io/github/forks/Manikanta-04/Student_Salary_prediction?style=flat-square&color=6366f1) |
| [🌌 Galaxy_relay-themed_online_chat](https://github.com/Manikanta-04/Galaxy_relay-themed_online_chat) | ![Stars](https://img.shields.io/github/stars/Manikanta-04/Galaxy_relay-themed_online_chat?style=flat-square&color=6366f1) | ![Forks](https://img.shields.io/github/forks/Manikanta-04/Galaxy_relay-themed_online_chat?style=flat-square&color=6366f1) |

</div>

---

<div align="center">

### 👀 Profile Views

![Profile Views](https://komarev.com/ghpvc/?username=Manikanta-04&color=6366f1&style=for-the-badge&label=PROFILE+VIEWS)

---

**💼 Open to SDE / Full Stack / ML Engineer roles — 2027 Batch**

*If my work impressed you — let's connect on [LinkedIn](https://linkedin.com/in/manikanta-naripeddi-4326232a5/) or drop an [email](mailto:manikantachowdary296@gmail.com)!*

<img src="https://capsule-render.vercel.app/api?type=waving&color=gradient&customColorList=6,11,20&height=100&section=footer"/>

</div>
